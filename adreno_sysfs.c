...
static int _gpulayers_llc_slice_enable_store(struct adreno_device *adreno_dev,
		bool val)
{
	if (!IS_ERR_OR_NULL(adreno_dev->gpulayers_llc_slice))
		adreno_dev->gpulayers_llc_slice_enable = val;
	return 0;
}

static bool _gpulayers_llc_slice_enable_show(struct adreno_device *adreno_dev)
{
	return adreno_dev->gpulayers_llc_slice_enable;
}
...
static ADRENO_SYSFS_BOOL(gpulayers_llc_slice_enable);
...
static const struct attribute *_attr_list[] = {
	...
	&adreno_attr_gpulayers_llc_slice_enable.attr.attr,
